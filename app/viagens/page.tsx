...
export default function ViagensMarcadas() {
  ...
  const fetchViagens = async () => {
    try {
      ...
      const viagensList = viagensSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); // Mapeia os documentos para um array de objetos
              ...
              <div key={viagem.id} className="w-[640px] rounded-xl py-5 px-6 shadow-shape bg-zinc-700 space-y-5 mb-4 relative">
                <div className="space-y-1.5">
                  ...
                  <a href="#" className="block text-xs text-zinc-400 truncate hover:text-zinc-200">
                    {`${viagem.startDate.toDate().toLocaleDateString()} até ${viagem.endDate.toDate().toLocaleDateString()}`}
                  </a>
                </div>
